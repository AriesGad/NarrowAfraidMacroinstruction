...
export default function HomeScreen() {
  ...
  const { status, accessRemainingSeconds, isAccessExpired, servers, tweaks, selectedServerId, selectedProfileId, fastestStatus, fastestMessage, settings, selectServer, selectProfile, toggleSetting, clearAppData, requestConnection, disconnect, logs, clearLogs } = useAppState();
  ...
  return <Screen>
    ...
    <Modal visible={menuOpen} transparent animationType="none" onRequestClose={() => setMenuOpen(false)}><View style={styles.drawerRoot}><Pressable style={styles.drawerBackdrop} onPress={() => setMenuOpen(false)} /><View style={[styles.drawer, { backgroundColor: colors.card, paddingTop: insets.top + 14 }]}><View style={styles.drawerHeader}><View style={[styles.drawerMark, { backgroundColor: colors.primary }]}><Ionicons name="shield-checkmark" size={22} color={colors.primaryForeground} /></View><View style={{ flex: 1 }}><Text style={[styles.drawerBrand, { color: colors.foreground }]}>ARIES TUNNEL</Text><Text style={[styles.drawerSubtitle, { color: colors.mutedForeground }]}>VPN CONTROL CENTER</Text></View><Pressable onPress={() => setMenuOpen(false)} hitSlop={10}><Ionicons name="close" size={21} color={colors.mutedForeground} /></Pressable></View><DrawerToggle icon="code-slash-outline" label="Custom Tweak" value={settings.customTweak} onChange={() => toggleSetting('customTweak')} colors={colors} /><DrawerToggle icon="git-network-outline" label="Forward DNS" value={settings.forwardDns} onChange={() => toggleSetting('forwardDns')} colors={colors} /><DrawerToggle icon="share-social-outline" label="Forward UDP" value={settings.forwardUdp} onChange={() => toggleSetting('forwardUdp')} colors={colors} /><DrawerToggle icon="hardware-chip-outline" label="CPU Wakelock" value={settings.cpuWakelock} onChange={() => toggleSetting('cpuWakelock')} colors={colors} /><DrawerAction icon="wifi-outline" label="Hotspot Share" colors={colors} onPress={() => { setMenuOpen(false); router.push('/hotspot'); }} /><View style={[styles.drawerDivider, { backgroundColor: colors.border }]} /><DrawerAction icon="location-outline" label="Check IP Address" colors={colors} onPress={() => { setMenuOpen(false); router.push('/response-checker'); }} /><DrawerAction icon="paper-plane-outline" label="Join Us on Telegram" colors={colors} onPress={() => showLinkMessage('Telegram')} /><DrawerAction icon="globe-outline" label="Visit our Blog" colors={colors} onPress={() => showLinkMessage('Blog')} /><DrawerAction icon="logo-google-playstore" label="More VPN Apps" colors={colors} onPress={() => { setMenuOpen(false); router.push('/(tabs)/servers'); }} /><DrawerAction icon="trash-outline" label="Clear App data" colors={colors} onPress={() => Alert.alert('Clear app data?', 'This resets access time, logs, selections, and local preferences.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Clear', style: 'destructive', onPress: () => { clearAppData(); setMenuOpen(false); } }])} /><DrawerAction icon="warning-outline" label="Report Bug" colors={colors} onPress={() => showLinkMessage('Bug reporting')} /><Text style={[styles.drawerFooter, { color: colors.mutedForeground }]}>Only use authorized VPN endpoints and credentials.</Text></View></View></Modal>
  </Screen>;
}
...
function showLinkMessage(label: string) {
  Alert.alert(label, 'An official Aries Tunnel destination has not been configured yet.');
}
