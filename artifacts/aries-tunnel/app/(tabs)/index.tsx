...
import { Header, IconButton, Label, Pill, PrimaryButton, Screen, Surface } from '@/components/Primitives';
...
export default function HomeScreen() {
  ...
  const { status, accessRemainingSeconds, isAccessExpired, servers, selectedServerId, requestConnection, disconnect, logs, clearLogs } = useAppState();
  ...
  return <Screen>
    <View style={{ paddingTop: Math.max(insets.top - 14, 0) }}>
      ...
      <Surface style={styles.serverCard}>
        <View style={styles.cardHeading}><Label>Selected server</Label><Pill tone={selectedServer.available ? 'green' : 'amber'}>{selectedServer.available ? selectedServer.ping + ' ms' : 'CUSTOM'}</Pill></View>
        <View style={styles.serverLine}><View style={[styles.serverFlag, { backgroundColor: colors.secondary }]}><Ionicons name="globe-outline" size={19} color={colors.primary} /></View><View style={{ flex: 1 }}><Text style={[styles.serverName, { color: colors.foreground }]}>{selectedServer.name}</Text><Text style={[styles.serverLocation, { color: colors.mutedForeground }]}>{selectedServer.location}</Text></View><Ionicons name="chevron-forward" size={18} color={colors.mutedForeground} /></View>
      </Surface>
      ...
      {unavailable ? <View style={[styles.notice, { backgroundColor: colors.accent, borderColor: colors.border }]}><Ionicons name="information-circle-outline" size={19} color={colors.primary} /><Text style={[styles.noticeText, { color: colors.accentForeground }]}>The UI is ready, but no real VPN protocol is configured. Aries Tunnel will never claim a connection until an authorized backend is added.</Text></View> : null}
    </View>
    <ModalShell visible={rewardOpen} title="Earn VPN access" subtitle="Watch a rewarded ad to receive 4 hours of VPN access. Time is only granted after the official rewarded-ad SDK confirms that the reward was earned." onClose={() => setRewardOpen(false)}>
      ...
      <PrimaryButton title="REWARDED AD NOT CONNECTED" icon="lock-closed-outline" disabled onPress={async () => { const earned = await RewardManager.showRewardedAd(); if (earned) setRewardOpen(false); }} />
    </ModalShell>
    <ModalShell visible={logOpen} title="VPN log" subtitle="Secrets are never written to this log." onClose={() => setLogOpen(false)}>
      <View style={styles.logToolbar}><Text style={[styles.logCount, { color: colors.mutedForeground }]}>{logs.length} events</Text><Pressable onPress={clearLogs}><Text style={[styles.clearText, { color: colors.primary }]}>CLEAR LOG</Text></Pressable></View>
...
const styles = StyleSheet.create({
  heroIntro: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  introText: { fontFamily: 'Inter_400Regular', fontSize: 12 },
  connectArea: { alignItems: 'center', justifyContent: 'center', paddingVertical: 30 },
  outerRing: { width: 218, height: 218, borderRadius: 140, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  connectButton: { width: 186, height: 186, borderRadius: 100, alignItems: 'center', justifyContent: 'center', gap: 7 },
  connectLabel: { color: '#ffffff', fontFamily: 'Inter_700Bold', letterSpacing: 1.4, fontSize: 19 },
  connectStatus: { color: 'rgba(255,255,255,0.78)', fontFamily: 'Inter_500Medium', fontSize: 11 },
  timerHeader: { alignItems: 'center', marginBottom: 16 },
  timerValue: { fontFamily: 'Inter_700Bold', fontSize: 35, letterSpacing: 1.5, marginTop: 6 },
  accessCard: { gap: 18 },
  accessTop: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  accessIcon: { width: 44, height: 44, borderRadius: 15, alignItems: 'center', justifyContent: 'center' },
  accessTitle: { fontFamily: 'Inter_700Bold', fontSize: 15 },
  accessSubtitle: { fontFamily: 'Inter_400Regular', fontSize: 12, lineHeight: 18, marginTop: 4 },
  serverCard: { marginTop: 14 },
  cardHeading: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 15 },
  serverLine: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  serverFlag: { width: 42, height: 42, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  serverName: { fontFamily: 'Inter_700Bold', fontSize: 14 },
  serverLocation: { fontFamily: 'Inter_400Regular', fontSize: 12, marginTop: 4 },
  statsCard: { marginTop: 14, flexDirection: 'row', justifyContent: 'space-between' },
  statValue: { fontFamily: 'Inter_700Bold', fontSize: 16, marginTop: 7 },
  notice: { flexDirection: 'row', gap: 10, padding: 14, borderRadius: 17, borderWidth: 1, marginTop: 14 },
  ...
  logToolbar: { flexDirection: 'row', justifyContent: 'space-between', marginVertical: 16 },
  logCount: { fontFamily: 'Inter_500Medium', fontSize: 12 },
  clearText: { fontFamily: 'Inter_700Bold', fontSize: 11, letterSpacing: 0.7 },
  logList: { borderWidth: 1, borderRadius: 16, padding: 12, maxHeight: 280 },
  logRow: { paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.06)' },
  logType: { fontFamily: 'Inter_700Bold', fontSize: 10, letterSpacing: 0.8 },
  logMessage: { fontFamily: 'Inter_400Regular', fontSize: 12, marginTop: 4, lineHeight: 17 },
